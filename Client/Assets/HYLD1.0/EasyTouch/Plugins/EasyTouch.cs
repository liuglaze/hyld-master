...
/// 		- Add Logging.HYLDDebug.LogError if no camera with flag MainCamera was found in the scene
/// 
/// 	* EasyTouch class
/// 	-----------------
		...
		if (touchCameras.Count>0){
			for (int i=0;i<touchCameras.Count;i++){
				if (touchCameras[i].camera!=null && touchCameras[i].camera.enabled){
			        ...
			        if( UnityEngine.Physics.Raycast( ray, out hit,float.MaxValue,mask ) ){
						finger.pickedCamera = touchCameras[i].camera;
						finger.isGuiCamera = touchCameras[i].guiCamera;
			...
			Logging.HYLDDebug.LogWarning("No camera is assigned to EasyTouch");	
		}
	...
	private bool IsTouchHoverNGui(int touchIndex){
		...
		if (enabledNGuiMode){
			...
			while (!returnValue && i<nGUICameras.Count){
				...
				returnValue =  UnityEngine.Physics.Raycast( ray, out hit,float.MaxValue,mask );
