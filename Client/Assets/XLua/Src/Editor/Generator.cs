    ...
    public static class Generator
    {
...
#else
                Logging.HYLDDebug.LogError("gen wrap file fail! err=" + e.Message + ", stack=" + e.StackTrace);
#endif
            }
            finally
            {
        ...
        static void GenWrap(IEnumerable<Type> types, string save_path)
        {
            ...
            var typeMap = types.ToDictionary(type => {
                //Logging.HYLDDebug.Log("type:" + type);
...
#pragma warning restore 618
        ...
        public static void GenAll()
        {
            ...
            GenLuaRegister();
            callCustomGen();
            Logging.HYLDDebug.Log("finished! use " + (DateTime.Now - start).TotalMilliseconds + " ms");
            AssetDatabase.Refresh();
        }
        ...
        public static void GenUsingCLI()
        {
            ...
            {
                Logging.HYLDDebug.LogError(process.StandardError.ReadLine());
            }

            while (!process.StandardOutput.EndOfStream)
            {
                Logging.HYLDDebug.Log(process.StandardOutput.ReadLine());
            }
        ...
        public static void CustomGen(string template_src, GetTasks get_tasks)
        {
            ...
            }))
            {
                ...
                {
                    Logging.HYLDDebug.LogError("gen file fail! template=" + template_src + ", err=" + e.Message + ", stack=" + e.StackTrace);
                }
                finally
                {
